...
class FitT2():
	...
	def __init__(self, data, procpar, fitmethod, skipfirstecho):
		...
		if skipfirstecho == False:
			...
			self.data = data[:,:,:,1:]
			self.echo_times = [i*te for i in range(2,ne+1)] # echo times in seconds	
		...
		############################## MONO EXP WITHOUT CONST #####################################
			def fit1D(data1D, echo_times):		
				...
				try:
					...
					popt, pcov = curve_fit(f, x, y, p0=(10000.0, 0.05), check_finite=False, \
		...
		############################## MONO EXP WITH CONST #####################################
			def fit1D(data1D, echo_times):
				...
				try:
					...
					popt, pcov = curve_fit(f, x, y, p0=(10000.0, 0.05, 50.0), check_finite=False, \
...
if __name__ == '__main__':
	...
	parser.add_argument('--skipfirstecho',\
						dest='skipfirstecho',\
						help='skip first echo',\
						default=True)
	...
	fitter = FitT2(data, procpar, args.fitmethod, args.skipfirstecho)
	out = fitter.fit()
